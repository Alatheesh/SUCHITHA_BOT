...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[           
            InlineKeyboardButton(⚡𝕄𝔸𝕀ℕ⚡', url=f'https://t.me/{SUPPORT_CHAT}')
            ],[
            InlineKeyboardButton('🌀 USE ME IN PM 🌀', url=f"https://t.me/{temp.U_NAME}?start=help")
            ]]
...
@Client.on_message(filters.command('getlogs') & filters.user(ADMINS))
...
async def delete(bot, message):
    ...
    if reply and reply.media:
        ...
        await message.reply('Reply to file with /delfile which you want to delete', quote=True)
        return
...
@Client.on_message(filters.command('deldb') & filters.user(ADMINS))
...
async def settings(client, message):
    ...
    if not userid:
        return await message.reply(f"You are anonymous admin. Use /gconnect {message.chat.id} in PM")
